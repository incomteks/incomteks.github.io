...
?>
<tr>
<td><b><?=$armn[$i]?></b><br>ранг: <b><?=$wins[$i]?></b>, уровень атаки: <b><?=$techatt[$i]?></b>, уровень защиты: <b><?=$techarm[$i]?></b></td>
<td><?=$linf[$i]?></td>
<td><?=$hinf[$i]?></td>
<td><?=$tank[$i]?></td>
<td><?=$rtank[$i]?></td>
...
$coef=3/rand(2,6);
$damage=$damage*$unit_q[$i]*(1+$wins[$j]/50)*$coef;
...
if ($target!=0) echo "Отряд ".$war_n[$unit_n[$i]]." атакует отряд ".$war_n[$target].". Нанесено ".ceil($totaldmg)." единиц урона.<br>";
...
if ($x==1)
{
$x2=mysql_query("update `army` set `wins`='$setwins',`linf`='$linf[$i]',`hinf`='$hinf[$i]',`tank`='$tank[$i]',`rtank`='$rtank[$i]',`spec1`='$sard[$i]',`spec2`='$deva[$i]',`spec3`='$free[$i]' where `n`='$army2_n'");
if ($x==1)
{
